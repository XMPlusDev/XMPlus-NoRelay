...
install_XMPlus() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/XMPlusDev/XMPlus-NoRelay/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XMPlus/XMPlus-linux.zip https://github.com/XMPlusDev/XMPlus-NoRelay/releases/download/${last_version}/XMPlus-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/XMPlusDev/XMPlus-NoRelay/releases/download/${last_version}/XMPlus-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/XMPlusDev/XMPlus-NoRelay/install/XMPlus.service"
    ...
    curl -o /usr/bin/XMPlus -Ls https://raw.githubusercontent.com/XMPlusDev/XMPlus-NoRelay/install/XMPlus.sh
