...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/XMPlusDev/XMPlus-NoRelay/install/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XMPlus -N --no-check-certificate https://raw.githubusercontent.com/XMPlusDev/XMPlus-NoRelay/install/XMPlus.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/XMPlusDev/XMPlus-NoRelay ---
